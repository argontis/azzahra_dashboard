...
<?php
  ...
  $suksesMsg = session('sukses');
  $gagalMsg = session('gagal');
...
<div class="page-header">
    <div class="flex items-center gap-2 text-white">
        ...
        <span class="hidden md:inline-block text-white text-xs opacity-90 ml-4 border-l border-white/20 pl-4">Manage orders</span>
    </div>
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="notification-container flex items-center justify-center relative w-9 h-9 bg-white rounded-md text-gray-600 hover:bg-gray-100 transition-colors shadow-sm cursor-pointer" onclick="toggleNotificationDropdown()">
    ...
    <!-- Notifikasi Dropdown -->
    <div class="notification-dropdown absolute right-4 top-14 bg-white shadow-lg rounded-md w-80 text-gray-800" id="notificationDropdown" style="display: none; border: 1px solid #e2e8f0;">
    ...
    <div class="col-span-12 lg:col-span-9">
      <div class="box p-6">
            ...
            <span class="px-3 py-1 rounded-full text-sm font-semibold
              ...
              <?php
                if ($isRepairing && isset($total_repairing)) {
                  ...
                  echo isset($orders) && is_object($orders) ? $orders->count() : 0;
                } else {
                  echo $orders->count();
                }
              ?> Orders
              ...
              <?php foreach ($orders as $row) : ?>
                ...
                <div class="border border-blue-200 rounded-lg p-4 hover:shadow-xl transition-all duration-200 bg-white cursor-pointer hover:border-blue-400"
                  ...
                  <div class="bg-blue-50 rounded-lg p-3 mb-3">
                    ...
                    <p class="text-sm font-bold text-blue-900"><?= $row->tdkn_barang ?? 'N/A' ?></p>
                  </div>
        ...
        <?php elseif ($isRepairing): ?>
          <!-- Search Form -->
          <div class="mb-6">
            <form method="get" action="{{ route('admin.order.index', 'repairing') }}" class="flex gap-2" onsubmit="showLoading()">
        ...
        <?php elseif ($isCompleted): ?>
          <!-- Search Form -->
          <div class="mb-6">
            <form method="get" action="{{ route('admin.order.index', 'completed') }}" class="flex gap-2" onsubmit="showLoading()">
        ...
        <?php elseif ($isFailed): ?>
          <!-- Search Form -->
          <div class="mb-6">
            <form method="get" action="{{ route('admin.order.index', 'failed') }}" class="flex gap-2" onsubmit="showLoading()">
        ...
        <?php elseif ($isOrderPart): ?>
          <!-- Search Form -->
          <div class="mb-6">
            <form method="get" action="{{ route('admin.order.index', 'orderpart') }}" class="flex gap-2">
          ...
          <?php if ($orders && ((is_object($orders) && $orders->count() > 0) || (is_array($orders) && count($orders) > 0))): ?>
            <div class="grid grid-cols-1 gap-4">
              <?php
              $orders_array = is_object($orders) ? $orders : $orders;
              ...
              <div class="custom-modal" id="<?= $modalId ?>" style="display: none; background: transparent; margin-top: 50px;">
                ...
                <div class="custom-modal-content modal__content--md p-0" style="max-height: 95vh;">
                  <div class="bg-white rounded-lg overflow-hidden">
                    ...
                    <!-- Form -->
                    <form method="post" action="{{ route('admin.order.update_part_marking') }}">
                      <input type="hidden" name="trans_kode" value="<?= $row->trans_kode; ?>">
                          ...
                          <?php if (!empty($row->tindakan)): ?>
                            <?php foreach ($row->tindakan as $t): ?>
                              <div class="bg-white rounded-lg p-3 mb-2">
                                <p class="font-semibold text-gray-900"><?= $t->tdkn_barang; ?></p>
  ...
  <?php if ($isPending && isset($orders)): ?>
    <?php foreach ($orders as $row) : ?>
        <?php
            ...
            $hasApprovalSection = ($row->tdkn_barang == 'Penggantian Sparepart');
        ?>
    ...
    <div class="custom-modal" id="<?= $confirmModalId ?>" style="display: none;">
      ...
      <div class="custom-modal-content modal__content--xl p-0">
        <div class="bg-white rounded-lg overflow-hidden" style="max-height: 90vh; overflow-y: auto;">
          ...
          <!-- Form -->
          <form method="post" action="{{ route('admin.order.confirm_order') }}">
            <input type="hidden" name="trans_kode" value="<?= $row->trans_kode; ?>">
            ...
            <div class="p-6">
              ...
              <!-- Pilih Karyawan -->
              <?php if (isset($karyawan) && $karyawan->count() > 0): ?>
                <div>
    ...
    <div class="custom-modal" id="<?= $repairingModalId ?>" style="display: none;">
      ...
      <div class="custom-modal-content modal__content--md p-0">
        <div class="bg-white rounded-lg overflow-hidden" style="max-height: 90vh; overflow-y: auto;">
          ...
          <!-- Form -->
          <form method="post" action="{{ route('admin.order.service_complete') }}">
            <input type="hidden" name="trans_kode" value="<?= $row->trans_kode; ?>">
            ...
            document.querySelectorAll('input[name="service_status"]').forEach(radio => {
              radio.addEventListener('change', function() {
                ...
                if (this.value === 'failed') {
                  form.action = '{{ route('admin.order.service_failed') }}';
                  ...
                  form.action = '{{ route('admin.order.service_complete') }}';
...
window.submitApproval = function(transIdSanitized, action) {
  ...
  if (formData.has('subtot')) {
    if (hasApproval === '1' && barangTersedia === '0') {
      actionUrl = '{{ route('admin.order.inform_unavailable') }}'; // Kirim pesan tidak tersedia
    } else {
      actionUrl = '{{ route('admin.order.approve_order') }}'; // Update subtot dan status ke approved
    }
  } else if (formData.has('sisa')) {
    actionUrl = '{{ route('admin.order.update_trans_total') }}'; // Update trans_total with sisa
  }
...
window.confirmReject = function(transKode) {
  ...
  const doReject = () => {
    ...
    form.method = 'POST';
    form.action = '<?= url('Order/reject_order/') ?>' + transKode;
...
window.sendFollowUp = function(hp, nama, cos_kode, merek, device) {
  ...
  // window.location.href = '<?= url('Order/mark_followup_sent/') ?>' + trans_kode;
};
